...
export class GuestPage {
  ...
  coachAthletes() {
    var q = "select c.name, ap.name from person ap, coaches co where co.AID = ap.ID AND co.CID NOT IN ((SELECT co2.CID FROM " +
  ...
  coachName2 = "";
  coachAwards() {
    var q = "select c.name, ap.AwardName, ap.year from AwardPerson ap, Coach c, Person p where p.name = \'" + this.coachName2 + "\' and c.ID = p.ID and ap.ID = c.ID";
    console.log(q);
    this.myApp.retrieveQueryData(q).then((data)=> {
      this.myApp.displayQueryData(data, "otherResult");
      console.log(data);
